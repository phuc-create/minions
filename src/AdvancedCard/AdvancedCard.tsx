...
const AdvancedCard: React.FC<AdvancedCardProps> = ({ cardinfor, colors, styled }) => {
  const color = colors ? colors : [
    '#0984e3', '#00b894', '#d63031', '#6c5ce7', '#EE5A24', '#ED4C67'
  ]
  const style = { '--progress': cardinfor.progress + '%', backgroundColor: color[Math.floor(Math.random() * color.length)] }
  return (
    <div className="list__bx" style={styled}>
      ...
      <div className="list__bx--time">
        <div className="list__bx--time-team">
          {
            cardinfor.team && cardinfor.team.map((t: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined, i: React.Key | null | undefined) => {
              return (
                <span key={i}>{t}</span>
              )
            })
        ...
        <div className="list__bx--time-deadline">
          {cardinfor.deadline > 1 ? cardinfor.deadline + ' days left' : cardinfor.deadline + ' day left'}
        </div>
      </div>
    </div>
